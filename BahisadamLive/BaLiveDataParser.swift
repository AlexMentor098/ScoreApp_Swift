		...
		if let league = leagueObject.value as? Dictionary<String, AnyObject> {
			...
			if let matches = league["matches"] as? [Dictionary<String, AnyObject>] {
				...
				for match in matches {
					...
					let matchData = BA_Matc(homeTeam: homeTeamData, awayTeam: awayTeamData, resultType: resultType, homeGoals: Int(splittedResult[0])!, awayGoals: Int(splittedResult[1])!, matchId: matchId, matchYear: matchYear, currentMinutes: currentMinutes)
